...
>(client: Client<transport, chain, account>): Address {
  ...
  const chainConfig = config[client.chain.id.toString()];
  if (!chainConfig) {
    throw new Error(`USDC configuration not found for chain ID: ${client.chain.id}`);
  }
...
export function getUsdcAddressForChain(chainId: number): Address {
  ...
  if (!chainConfig) {
    throw new Error(`USDC configuration not found for chain ID: ${chainId}`);
  }
  return chainConfig.usdcAddress as Address;
}
