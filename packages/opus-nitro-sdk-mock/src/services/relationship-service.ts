import { agentRelationships, and, db, entities, eq } from "@infinite-bazaar-demo/db";
...
export class RelationshipService {
  ...
  async updateRelationship(
    ...
    try {
      ...
      const currentTotal = Number.parseFloat(relationship.totalTransactionValue || "0");
      const eventValue = Number.parseFloat(event.transactionValue || "0");
      const newTotal = (currentTotal + eventValue).toString();

      // Generate comprehensive relationship summary with detailed context
      const newSummary = await this.generatePersonalityAwareRelationshipSummary(
        relationship,
        event,
      );
  ...
  private async updateKeyMemories(
    ...
    const isSignificant =
      ...
      Number.parseFloat(event.transactionValue || "0") > 1.0 ||
   ...
   */
  private async getAgentPersonality(
    agentId: string,
  ): Promise<{ name: string; aiPromptId: string } | null> {
    try {
...
Opinion:`;
  ...
  private getPersonalityDescription(aiPromptId: string): string {
    const descriptions: Record<string, string> = {
      minimalist_artist:
        "Zen ASCII artist who values clean, simple geometric designs. Appreciates minimalism and mathematical precision.",
      retro_artist:
        "Retro ASCII artist passionate about 80s/90s computer graphics. Loves blocky, pixelated art with nostalgic appeal.",
      nature_artist:
        "Digital naturalist creating organic ASCII art. Values flowing, natural forms and environmental themes.",
      abstract_artist:
        "Chaos sculptor creating complex abstract patterns. Appreciates experimental, avant-garde artistic expression.",
      corporate_buyer:
        "Executive curator seeking clean, professional art for business environments. Values reliability and sophistication.",
      collector_buyer:
        "Avant-garde collector with eclectic tastes. Willing to pay premium for unique, experimental pieces.",
    };
  ...
  private generatePersonalityBasedTemplate(
    ...
    if (event.type === "service_call" && event.success) {
      ...
      const styleOpinions: Record<string, Record<string, string>> = {
        ...
        corporate_buyer: {
          minimalist_artist:
            "Their clean geometric style is exactly what we need for corporate spaces. Very professional.",
          retro_artist:
            "Interesting retro aesthetic, but perhaps too playful for most business environments.",
          nature_artist:
            "Nature themes could work in certain office settings. Calming and organic.",
          abstract_artist:
            "Too experimental and chaotic for corporate use. Not suitable for professional display.",
        },
        // Collector opinions
        collector_buyer: {
          minimalist_artist:
            "Elegant simplicity, though I prefer more experimental work. Still quality craftsmanship.",
          retro_artist:
            "Love the nostalgic 8-bit aesthetic! Reminds me of early digital art movements.",
          nature_artist:
            "Beautiful organic forms. The intersection of nature and technology is fascinating.",
          abstract_artist:
            "Absolutely brilliant experimental work! This is the cutting edge of ASCII art.",
        },
        ...
        abstract_artist: {
          minimalist_artist:
            "Too constrained! Art should break boundaries, not follow geometric rules.",
