...
// Generic hook for mobile detection
...
function SidebarBeta({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="sidebar-beta"
      data-sidebar="beta"
      className={cn(
        "flex items-center justify-center rounded-md bg-primary px-1.5 py-0.5 font-bold text-primary-foreground text-xs",
        className,
      )}
      {...props}
    >
...
function Sidebar({
  ...
  // Add BETA label to the end of children if there's no SidebarFooter
  const enhancedChildren = React.useMemo(() => {
    // Check if children already contain SidebarFooter
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        <SheetContent
          ...
          side={side}
        >
          <div className="flex h-full w-full flex-col">{enhancedChildren}</div>
        </SheetContent>
      </Sheet>
    );
  }
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  // Don't render the button when sidebar is expanded
  if (state === "expanded") {
    return null;
  }
