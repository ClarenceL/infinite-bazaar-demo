...
export function ChatContainer() {
  ...
  const fetchMessages = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_INFINITE_BAZAAR_API_URL || "http://localhost:3104"}/api/chat/messages`,
      );
  ...
  const fetchStreamingMessages = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_INFINITE_BAZAAR_API_URL || "http://localhost:3104"}/api/chat/messages?streaming=true`,
      );
  ...
  // Render individual message bubble (following reference styling)
  const renderMessage = (message: ChatMessage) => {
    const roleStyles = {
      user: "ml-10 bg-primary/10 border-white/20",
      assistant: "mr-10 bg-white/5 border-white/10",
      system: "bg-destructive/10 text-destructive border-red-500/20",
    };
    ...
    return (
      <div
        key={message.id}
        className={`rounded-lg border p-2 text-slate-300 text-sm backdrop-blur-sm ${roleStyles[message.role]}`}
      >
        <div className="mb-1 flex items-center justify-between">
          <span className="font-medium text-slate-400 text-xs">{roleLabel[message.role]}</span>
          {message.completedAt === null ? (
            <span className="text-xs text-yellow-400">streaming...</span>
          ) : message.completedAt && Date.now() - message.completedAt < 30000 ? (
            <span className="text-green-400 text-xs">just completed</span>
          ) : null}
        </div>
        {parsedContent.includes("<") ? (
          ...
          <div className="whitespace-pre-wrap text-sm leading-relaxed">{parsedContent}</div>
        )}
      </div>
    );
  };
    ...
    return (
      <div className="flex h-full flex-col">
        <div className="border-white/10 border-b bg-black/20 p-4 backdrop-blur-sm">
          <h3 className="font-semibold text-lg text-white">Live Chat with Opus</h3>
        </div>
  ...
  return (
    <div className="flex h-full flex-col">
      {/* Header */}
      <div className="border-white/10 border-b bg-black/20 p-4 backdrop-blur-sm">
        <h3 className="font-semibold text-base text-white">Live Chat with Opus</h3>
        ...
        {isLoading ? (
          ...
          <div className="flex h-full items-center justify-center">
            <div className="text-center text-slate-400">
              ...
              <div className="text-xs">Start a conversation with Opus!</div>
            </div>
          </div>
        ) : (
